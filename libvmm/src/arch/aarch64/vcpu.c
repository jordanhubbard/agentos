...
void vcpu_reset(size_t vcpu_id)
{
    ...
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_SCTLR, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_TTBR0, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_TTBR1, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_TCR, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_MAIR, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_AMAIR, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_CIDR, 0);
    /* other system registers EL1 */
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_ACTLR, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_CPACR, 0);
    /* exception handling registers EL1 */
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_AFSR0, 0);
     ...
     * bits in VMPIDR_EL2. */
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_VMPIDR_EL2, vcpu_id);
#endif /* CONFIG_MAX_NUM_NODES > 1 */
    /* general registers x0 to x30 have been saved by traps.S */
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_SP_EL1, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_ELR_EL1, 0);
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_SPSR_EL1, 0); // 32-bit
    /* generic timer registers, to be completed */
    vmm_vcpu_arm_write_reg(vcpu_id, seL4_VCPUReg_CNTV_CTL, 0);
...
void vcpu_print_regs(size_t vcpu_id)
{
    ...
    printf("    sctlr: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_SCTLR));
    printf("    ttbr0: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_TTBR0));
    printf("    ttbr1: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_TTBR1));
    printf("    tcr: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_TCR));
    printf("    mair: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_MAIR));
    printf("    amair: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_AMAIR));
    printf("    cidr: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_CIDR));
    /* other system registers EL1 */
    printf("    actlr: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_ACTLR));
    printf("    cpacr: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_CPACR));
    /* exception handling registers EL1 */
    printf("    afsr0: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_AFSR0));
...
#if CONFIG_MAX_NUM_NODES > 1
    /* Virtualisation Multiprocessor ID Register */
    printf("    vmpidr_el2: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_VMPIDR_EL2));
#endif
    /* general registers x0 to x30 have been saved by traps.S */
    printf("    sp_el1: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_SP_EL1));
    printf("    elr_el1: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_ELR_EL1));
    printf("    spsr_el1: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_SPSR_EL1)); // 32-bit // @ivanv what
    /* generic timer registers, to be completed */
    ...
    printf("    cntkctl_el1: 0x%016lx\n", vmm_vcpu_arm_read_reg(vcpu_id, seL4_VCPUReg_CNTKCTL_EL1));
}
