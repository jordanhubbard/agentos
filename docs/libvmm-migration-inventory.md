...
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:113` | `void init(void)` — Microkit PD entry | `main()` loop calling `seL4_ReplyRecv` | M | Both x86 and AArch64 stubs use this; must become a `seL4_ReplyRecv` server loop |
| `guest_vmm.c:122` | `void notified(microkit_channel ch)` — async notification handler | Dispatch on badge inside `seL4_ReplyRecv` loop | M | Called per-channel; replace with badged notification cap + dispatch table |
| `guest_vmm.c:628` | `seL4_Bool fault(microkit_child, microkit_msginfo, microkit_msginfo*)` — fault handler | `seL4_Fault_*` label dispatch inside `seL4_ReplyRecv` | M | `fault_handle()` already unpacks `microkit_msginfo_get_label`; see §2 |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:305` | `microkit_ppcall(SERIAL_PD_CH, microkit_msginfo_new(MSG_SERIAL_OPEN, 2))` | `seL4_Call(serial_ep_cap, msginfo)` | S | SERIAL_PD_CH=1; needs a badged endpoint cap for serial_pd |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:511` | `microkit_notify(CONTROLLER_CH)` — notify controller VMM ready | `seL4_Send(controller_notif_cap, seL4_MessageInfo_new(0,0,0,0))` | S | CONTROLLER_CH=2 |
| `guest_vmm.c:595` | `microkit_notify(CONTROLLER_CH)` — notify GPU result ready | Same as above | S | |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:344` | `microkit_irq_ack(VIRTIO_NET_IRQ_CH)` — ack virtio-net IRQ (ch=5) | `seL4_IRQHandler_Ack(virtio_net_irq_handler_cap)` | S | Must obtain `seL4_IRQControl_Get` cap from root-task first |
| `guest_vmm.c:350` | `microkit_irq_ack(VIRTIO_BLK_IRQ_CH)` — ack virtio-blk IRQ (ch=6) | `seL4_IRQHandler_Ack(virtio_blk_irq_handler_cap)` | S | Same pattern |
| `guest_vmm.c:480` | `microkit_irq_ack(VIRTIO_NET_IRQ_CH)` — init-time pre-ack | Same | S | |
| `guest_vmm.c:488` | `microkit_irq_ack(VIRTIO_BLK_IRQ_CH)` — init-time pre-ack | Same | S | |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:303–308` | `microkit_mr_set(0, ...)` / `microkit_msginfo_new(...)` / `microkit_mr_get(0)` | `seL4_SetMR(0, val)` / `seL4_MessageInfo_new(label,0,0,count)` / `seL4_GetMR(0)` | S | Pattern used throughout vmm_mux for all IPC calls |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:92,109,...` | `microkit_dbg_puts(...)` — many call sites in x86 stub + AArch64 | `seL4_DebugPutChar(c)` loop; or route via serial_pd | S | Mechanical; keep as debug helper, just swap the implementation |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:211` | `#define SERIAL_PD_CH 1` — channel to serial_pd | Named endpoint cap slot in CSpace | S | All CH_ defines become cap slot indices passed in at init |
| `guest_vmm.c:213` | `#define CONTROLLER_CH 2` | Same | S | |
| `guest_vmm.c:217,220` | `#define VIRTIO_NET_IRQ_CH 5`, `#define VIRTIO_BLK_IRQ_CH 6` | `seL4_IRQHandler` caps | S | |
| `guest_vmm.c:225,226` | `#define GPU_SHMEM_NOTIFY_IN_CH 3`, `GPU_SHMEM_NOTIFY_OUT_CH 4` | Notification cap indices | S | |
...
| File:Line | API call / assumption | seL4 equivalent | Effort | Notes |
|-----------|----------------------|-----------------|--------|-------|
| `guest_vmm.c:239,244` | `uintptr_t guest_ram_vaddr` — set by Microkit linker from `setvar_vaddr` | Root-task must pass vaddr via IPC at init, or via a known slot in shared memory | M | All `setvar_vaddr` symbols need an explicit init-time handoff |
| `guest_vmm.c:244,248` | `gpu_tensor_buf_vaddr`, `serial_shmem_linux_vaddr` (weak symbols) | Same mechanism | M | |
...
| `guest_vmm.c:371,378` | `seL4_TCB_SetAffinity` (comment, not yet wired) | Correct seL4 API when implemented |
...
- **`guest_vmm.c` commented-out code** at lines 293–337 (binding protocol TODOs) already shows the intended Microkit ppcall pattern — do not implement those TODOs; implement the seL4_Call equivalents directly.
