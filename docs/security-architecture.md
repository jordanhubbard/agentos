...
## Optional AArch64 graphics and input paths

The graphics composition is generated by `kernel/agentos-root-task/Makefile`:
`GUEST_GRAPHICS` adds `framebuffer_queue`, `GUEST_INPUT` adds `input_virt`, and
`DISPLAY_RAMFB` optionally adds the hardware-facing `display_ramfb` PD. These
services are not present in every default or dual-guest image.

```mermaid
flowchart LR
  guest[Linux guest DRM and evdev]
  vmm[VMM<br/>virtio-gpu and virtio-input emulation<br/>GPA bounds checks]
  fb[framebuffer_queue PD<br/>private surfaces<br/>per-client queues]
  input[input_virt PD<br/>per-client event queues<br/>no hardware authority]
  cc[cc_pd<br/>privileged control frontend]
  gui[External agentos_gui<br/>binary Unix socket<br/>optional SSH forwarding]
  display[Optional display_ramfb PD<br/>exclusive fw_cfg MMIO<br/>private DMA and scanout banks]
  hardware[QEMU display stand-in]
  guest <-->|faulting emulated device interface| vmm
  vmm <-->|own surface queue| fb
  cc -->|bounded observer requests| fb
  fb -->|immutable snapshot batches| cc
  gui <-->|generation-synchronized CC framing| cc
  cc -->|resolved guest handle and bounded events| input
  input -->|private keyboard and pointer queue| vmm
  fb -->|committed client-zero rectangle| display
  display --> hardware
```

The VMM receives its own client queues, not the framebuffer service's private
surface arena, another client's pages or display-driver mappings. The external
GUI receives copied frame data through CC, never mapped guest or service
memory. CC retains guest-control authority: its frame observer API is not an
independent read-only authorization boundary. SSH forwarding transports this
same privileged API rather than reducing its authority.

Two [managed graphics recreation runs](evidence/2026-09-19-spark/graphics-recreation-repeat.json)
at `adcf5a80` passed exact guest-written pixels, input packet and release
assertions, the original SSH identity and a persistent disk witness across
fresh guest handles. These qualify software QEMU paths. Optional RAMFB scanout
has separate `make test-display` and `make test-guest-display` evidence in
[`TCB.md`](TCB.md); neither establishes physical GPU support or simultaneous
peer display isolation. GUI main `41b6c1e` contains the tested native consumer;
the coordinated core contracts still await main integration.
