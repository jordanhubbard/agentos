...
| --- | --- | --- |
| x86 execution and firmware | [Two-vCPU Linux recreation](evidence/2026-09-19-spark/intel-current-smp.json) passed pinned SSH, CPU affinity and overlapping x87/SSE workers across two managed boots at `6df47ad`. This supersedes the earlier failed SMP attempt for that exact scope. The [two-GiB profile](evidence/2026-09-19-spark/intel-current-large-profile.json) also passed managed recreation at `cd9efb9`. | Final integrated qualification, measured memory capacity/stress, resource-admission boundaries and complete capability reclamation remain required. These profiles do not qualify Omarchy's illustrative 4-vCPU/8-GiB configuration. |
