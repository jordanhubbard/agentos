/*
 * agentctl — agentOS Interactive Launcher & Console Session Manager
 *
 ...
 *   Connects to a QEMU monitor socket to manage agentOS console sessions.
 *   Supports attach, detach, list, mode-change, and scroll.
 *
 * Build:
...
#define AGENTCTL_VERSION  "0.1.0"
#define MAX_CMD_LEN       4096
#define MONITOR_SOCK      "build/qemu_monitor.sock"
...
{
    bool sock_found = file_exists(MONITOR_SOCK);

    while (1) {
        clear();
        draw_header("Session Manager");
        draw_hints("  r Refresh   q Quit");
        ...
        if (!sock_found) {
            y += 2;
            attron(COLOR_PAIR(CP_DISABLED) | A_BOLD);
            mvprintw(y++, 4, "QEMU monitor socket not found:");
            attroff(COLOR_PAIR(CP_DISABLED) | A_BOLD);

            attron(COLOR_PAIR(CP_AVAIL));
            mvprintw(y++, 6, "%s", MONITOR_SOCK);
            ...
            /*
             * Socket exists. Display placeholder session list.
             * Full IPC implementation communicates over the controller
             * serial port using ASCII commands parsed by controller.c.
             */
            attron(A_BOLD);
            mvprintw(y++, 2, "Console sessions  (via %s)", MONITOR_SOCK);
            attroff(A_BOLD);
            y++;

            draw_box(y, 2, LINES - y - 5, COLS - 4, "Sessions");
            y += 2;

            attron(COLOR_PAIR(CP_DISABLED));
            mvprintw(y++, 4, "Session IPC bridge connecting...");
            mvprintw(y++, 4, "(send 'list' to controller serial port)");
            attroff(COLOR_PAIR(CP_DISABLED));
            y += 2;

            mvprintw(y++, 2, "Controller commands (over 2nd serial port):");
            attron(COLOR_PAIR(CP_DESC));
            mvprintw(y++, 4, "  list               — list active console sessions");
            mvprintw(y++, 4, "  attach <pd_id>     — attach to PD console (OP_CONSOLE_ATTACH)");
            mvprintw(y++, 4, "  detach             — detach current session (OP_CONSOLE_DETACH)");
            mvprintw(y++, 4, "  mode <0|1|2>       — set console mode (OP_CONSOLE_MODE)");
            mvprintw(y++, 4, "  scroll <n>         — scroll n lines (OP_CONSOLE_SCROLL)");
            mvprintw(y++, 4, "  status             — console_mux status (OP_CONSOLE_STATUS)");
            mvprintw(y++, 4, "  broadcast <msg>    — send to all sessions");
            attroff(COLOR_PAIR(CP_DESC));
            y++;

            mvprintw(y++, 2, "Keyboard (when attached):");
            attron(COLOR_PAIR(CP_DESC));
            mvprintw(y++, 4, "  Arrow keys  — select session    Enter — attach");
            mvprintw(y++, 4, "  d           — detach            b     — broadcast");
            mvprintw(y++, 4, "  PgUp/PgDn  — scroll            q     — quit");
            attroff(COLOR_PAIR(CP_DESC));
        }
        ...
        switch (ch) {
        case 'r': case 'R':
            sock_found = file_exists(MONITOR_SOCK);
            break;
...
{
    fprintf(stderr,
            "agentctl v%s — agentOS Interactive Launcher\n"
            "\n"
            "Usage:\n"
            "  %s              Pre-boot menu: select arch/board/guest, launch QEMU\n"
            "  %s -s           Post-boot session manager (console attach/detach)\n"
            ...
            "\n"
            "Session manager (-s):\n"
            "  Connects to QEMU monitor socket: %s\n"
            "  Shows active agentOS console sessions and allows attach/detach/scroll.\n"
            "\n"
            "Example:\n"
            "  make deps && make build TARGET_ARCH=aarch64\n"
            "  ./tools/agentctl/agentctl\n",
            AGENTCTL_VERSION, prog, prog, prog, prog, MONITOR_SOCK);
}
