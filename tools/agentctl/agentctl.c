...
static bool cc_device_status(uint32_t dev_type, uint32_t dev_handle,
                              cc_device_info_t *out)
{
    cc_reply_wire_t r;
    if (!ipc_send_recv(MSG_CC_DEVICE_STATUS, dev_type, dev_handle, 0,
                       NULL, 0, &r))
        return false;
    if (r.mr[0] != CC_OK) return false;
    memcpy(out, r.shmem, sizeof(*out));
...
static int batch_device_status(uint32_t dev_type, uint32_t dev_handle)
{
    if (!batch_connect()) return 1;
    cc_device_info_t info;
    memset(&info, 0, sizeof(info));
    bool ok = cc_device_status(dev_type, dev_handle, &info);
    cc_close_session();
    ipc_disconnect();

    if (!ok) {
        fprintf(stderr, "agentctl-ng: MSG_CC_DEVICE_STATUS failed\n");
        return 1;
    }
    printf("device: type=%s handle=0x%08x state=0x%08x\n",
           dev_type_label(info.dev_type), info.dev_handle, info.state);
    return 0;
}
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "  %s --batch device-status TYPE HANDLE  Per-device status\n"
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(batch_cmd, "device-status") == 0) {
        if (arg_idx + 2 > argc) {
            fprintf(stderr, "device-status: requires TYPE HANDLE\n"
                            "  TYPE: serial|net|block|usb|fb\n"); return 1;
        }
        const char *tn = argv[arg_idx];
        uint32_t dt;
        if      (strcmp(tn, "serial") == 0) dt = CC_DEV_TYPE_SERIAL;
        ...
        else if (strcmp(tn, "fb")     == 0) dt = CC_DEV_TYPE_FB;
        else { fprintf(stderr, "unknown device type: %s\n", tn); return 1; }
        uint32_t dh = (uint32_t)strtoul(argv[arg_idx + 1], NULL, 0);
        return batch_device_status(dt, dh);
    }
