 ...
 *
 * Provides AF_UNIX-equivalent local sockets via shared memory rings.
 * Each socket gets a 4KB slot in pflocal_shmem (16 slots × 4KB = 64KB).
 *
 * Socket lifecycle:
 ...
 *   pflocal_shmem[slot*4KB] — ring_header_t + ring data for each socket slot
 *
...
typedef struct {
    ...
    uint8_t  _pad;
    char     path[64];     /* bind path (empty if not bound) */
} pflocal_sock_t;
