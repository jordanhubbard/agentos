...
static void revoke_agent_caps(uint32_t agent_id, uint32_t reason_flag) {
    ...
    microkit_dbg_puts("[quota_pd] REVOKE agent=");
    put_dec(agent_id);
...
static void check_and_enforce(int slot_idx) {
    ...
    if (entry->cpu_limit_ms > 0 && entry->cpu_used_ms >= entry->cpu_limit_ms) {
        if (!(entry->flags & QUOTA_FLAG_CPU_EXCEED)) {
            ...
            microkit_dbg_puts("[quota_pd] CPU quota exceeded: agent=");
            put_dec(entry->agent_id);
            microkit_dbg_puts(" used=");
            put_dec(entry->cpu_used_ms);
            microkit_dbg_puts("ms limit=");
    ...
    if (entry->mem_limit_kb > 0 && entry->mem_used_kb >= entry->mem_limit_kb) {
        if (!(entry->flags & QUOTA_FLAG_MEM_EXCEED)) {
            ...
            microkit_dbg_puts("[quota_pd] MEM quota exceeded: agent=");
            put_dec(entry->agent_id);
            microkit_dbg_puts(" used=");
            put_dec(entry->mem_used_kb);
            microkit_dbg_puts("kb limit=");
...
static microkit_msginfo handle_request(microkit_msginfo msg) {
    ...
    case OP_QUOTA_REGISTER: {
        ...
        if (existing >= 0) {
            microkit_dbg_puts("[quota_pd] WARN: agent already registered, slot=");
        ...
        if (slot < 0) {
            microkit_dbg_puts("[quota_pd] ERROR: quota table full\n");
        ...
        microkit_dbg_puts("[quota_pd] Registered agent=");
        put_dec(agent_id);
    ...
    case OP_QUOTA_SET: {
        ...
        microkit_dbg_puts("[quota_pd] Updated agent=");
        put_dec(agent_id);
        microkit_dbg_puts(" cpu=");
        put_dec(new_cpu_ms);
        microkit_dbg_puts("ms mem=");
    ...
    default:
        microkit_dbg_puts("[quota_pd] WARN: unknown opcode\n");
...
void init(void) {
    ...
    quota_pd_init();
    microkit_dbg_puts("[quota_pd] Ready — priority 115, passive, ");
    put_dec(MAX_QUOTA_SLOTS);
    microkit_dbg_puts(" agent quota slots\n");
}
