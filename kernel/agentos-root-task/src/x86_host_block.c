...
#define SELECT(off) (UINT32_C(0x80002800) | (off))
static bool read32(seL4_CPtr cap, unsigned offset, uint32_t *value)
{
...
unsigned aos_x86_host_block_discover(aos_virtio_pci_layout_t *layout)
{
    *layout = (aos_virtio_pci_layout_t){0};
    ...
    for (unsigned i = 0; i < 64; i++) {
        if (!read32(cap, i * 4u, &snapshot[i])) goto out;
    }
    status = 3u;
    if (snapshot[0] != UINT32_C(0x10421af4)) goto out;
    status = 4u;
    disabled = true;
    if (!command(cap, (uint16_t)snapshot[1] & ~7u)) goto out;
    uint32_t current_command;
    if (!read32(cap, 4u, &current_command) || (current_command & 7u)) goto out;
    for (unsigned bar = 0; bar < 6; bar++) {
        ...
        bool ok = write32(cap, offset, UINT32_MAX);
        if (wide) ok = write32(cap, offset + 4u, UINT32_MAX) && ok;
        ok = read32(cap, offset, &mask_low) && ok;
        if (wide) ok = read32(cap, offset + 4u, &mask_high) && ok;
        /* Restore both halves even when a probe failed. */
        bool restored = write32(cap, offset, low);
        if (wide) restored = write32(cap, offset + 4u, snapshot[5u + bar]) && restored;
    ...
    status = aos_virtio_pci_decode((const uint8_t *)snapshot, sizes, 0x1042u, layout) ? 0u : 5u;
out:
    if (disabled) {
        /* Never re-enable decode or DMA after an unverified BAR restore. */
        for (unsigned bar = 0; bar < 6; bar++) {
            uint32_t value;
            if (!read32(cap, 0x10u + bar * 4u, &value) || value != snapshot[4u + bar])
                restore_command = false;
        }
        uint32_t restored_command;
        if (!restore_command || !command(cap, (uint16_t)snapshot[1]) ||
            !read32(cap, 4u, &restored_command) ||
