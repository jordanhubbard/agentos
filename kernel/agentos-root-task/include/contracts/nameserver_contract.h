 ...
 *   the controller maps this region and reads ns_entry_t[] after the call.
 *
 * Invariants:
 *   - All name fields are NUL-padded, not NUL-terminated; consumers must
 *     treat any byte past the first NUL as padding.
 *   - name[4] occupies MRs 5-8, each MR holds 4 bytes (little-endian), for
 *     a maximum service name of 16 bytes.
 ...
 *   - OP_NS_LIST writes up to (8192 / sizeof(ns_entry_t)) entries into
...
#define OP_NS_REGISTER       0x01u  /* register this PD's service endpoint */
#define OP_NS_LOOKUP         0x02u  /* look up a service by name */
#define OP_NS_LOOKUP_GATED   0x03u  /* look up — caller badge is validated */
#define OP_NS_UPDATE_STATUS  0x04u  /* update liveness/status of own entry */
#define OP_NS_LIST           0x05u  /* dump all entries into ns_registry_shmem */
#define OP_NS_DEREGISTER     0x06u  /* remove own service entry */
#define OP_NS_HEALTH         0x07u  /* return registered_count + NS_VERSION */

/* ─── Request structs ────────────────────────────────────────────────────── */

/* OP_NS_REGISTER
 * MR0=op, MR1=channel_id, MR2=pd_id, MR3=cap_classes, MR4=version, MR5-8=name[4]
 */
struct __attribute__((packed)) nameserver_req_register {
    ...
    uint32_t name[4];     /* service name: 16 bytes, NUL-padded, little-endian words */
};
...
struct __attribute__((packed)) nameserver_req_lookup {
    ...
    uint32_t name[4];     /* service name to search for */
};
...
struct __attribute__((packed)) nameserver_reply_register {
    uint32_t status;      /* enum nameserver_error */
};
...
/* ─── Shmem layout: ns_registry_shmem entry (OP_NS_LIST) ────────────────── */
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t version;
    uint32_t svc_status;
    uint32_t name[4];    /* 16-byte NUL-padded service name */
} ns_entry_t;

/* ─── Error codes ────────────────────────────────────────────────────────── */

enum nameserver_error {
    NS_OK             = 0,
    NS_ERR_BAD_ARGS   = 1,
    NS_ERR_DUPLICATE  = 2,
    NS_ERR_FULL       = 3,
