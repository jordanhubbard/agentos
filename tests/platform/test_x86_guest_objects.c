...
static const seL4_CPtr slots[] = {80u, 91u, 102u, 117u, 133u, 149u, 166u};
    ...
    seL4_Word depth, seL4_Word offset, seL4_Word count)
{
    ...
    seL4_CPtr ap_child=rebuild_test ? AOS_X86_AP_VCPU_POOL_CAP : 201u;
    assert(calls < AOS_X86_GUEST_OBJECT_COUNT+2u);
    assert(source == (!frame_test && calls==2u ? child :
                      !frame_test && calls==8u ? ap_child : pool));
    assert(root == (rebuild_test ? AOS_GUEST_RAM_SELF_CNODE : 7u));
    if (!frame_test && calls<2u) {
        assert(type==seL4_UntypedObject && bits==AOS_X86_VCPU_POOL_BITS &&
               offset==(calls==0u ? child : ap_child));
    } else {
        unsigned index=frame_test ? 0u : calls-2u;
...
{
    assert(calls == 10u && table == AOS_X86_GUEST_EPT_PDPT_CAP);
                            ...
                            seL4_Word gpa, seL4_Word attr)
{
    ...
    assert(table == directories[calls - 11u]);
    assert(gpa == addresses[calls - 11u]);
    assert(ept == AOS_GUEST_RAM_GUEST_VSPACE);
    assert(attr == seL4_X86_EPT_Default_VMAttributes);
    return ++calls == fail_at ? 19 : seL4_NoError;
}
...
{
    assert((bind_calls == 0u || bind_calls==2u) &&
           tcb == (bind_calls==0u ? AOS_X86_VMM_SELF_TCB_CAP : AOS_X86_AP_RUNNER_TCB_CAP) &&
...
{
    assert((bind_calls == 1u || bind_calls==3u) &&
           tcb == (bind_calls==1u ? AOS_X86_VMM_SELF_TCB_CAP : AOS_X86_AP_RUNNER_TCB_CAP) &&
           vcpu == AOS_GUEST_VCPU_CAP_BASE+(bind_calls==3u));
    return ++bind_calls == bind_failure ? 19 : seL4_NoError;
}
...
int main(void)
{
    ...
    for (pool = 50u; pool <= 51u; pool++) {
        for (fail_at = 0; fail_at <= AOS_X86_GUEST_OBJECT_COUNT+2u; fail_at++) {
            calls = 0;
            seL4_Error err = aos_x86_guest_objects_retype(pool, 7u, slots, 200u, 201u);
            assert(err == (fail_at ? 19 : seL4_NoError));
            assert(calls == (fail_at ? fail_at : AOS_X86_GUEST_OBJECT_COUNT+2u));
        }
    }
    ...
    for (fail_at = 0; fail_at <= 14u; fail_at++) {
        calls = 0;
        assert(aos_x86_guest_objects_rebuild() == (fail_at ? 19 : seL4_NoError));
        assert(calls == (fail_at ? fail_at : 14u));
    }
    for (bind_failure = 0; bind_failure <= 4; bind_failure++) {
        bind_calls = 0;
        assert(aos_x86_guest_objects_bind() == (bind_failure ? 19 : seL4_NoError));
        assert(bind_calls == (bind_failure ? bind_failure : 4u));
    }
    cpu_rebuild_test=1;
