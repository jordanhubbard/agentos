 ...
 * count<<32, MR4 media.  INFO replies capacity in sectors.
 */
                                  ...
                                  uint32_t nbytes)
{
    ...
    while (sectors_left > 0u) {
        ...
        rc = host_blk_call(media_id, op, sector, sectors, 0);
    ...
    void *ctx, aos_blk_virt_client_t *client, const aos_blk_req_t *req)
{
    ...
    case AOS_BLK_REQ_BARRIER:
        rc = host_blk_call(c->media_id, AOS_HOST_BLK_OP_FLUSH, 0u, 0u, 0);
        break;
    default:
        return AOS_BLK_RESP_ERR_INVALID_PARAM;
    }
