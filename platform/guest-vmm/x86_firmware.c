...
static bool have_payload_wait_snapshot;
...
void aos_x86_firmware_run(seL4_CPtr ep, seL4_Word result)
{
    ...
    for (unsigned exits = 0; ; exits++) {
        ...
        if (reason == 52u || reason == 7u) {
            ...
            /* Preserve the first delay after complete payload transfer: a
             * later shell key wait otherwise hides the loader's caller.
             * Without a payload, retain the latest pre-HLT PM polling site. */
            bool payload_read=config.boot.kernel_size &&
                config.boot_reads[0]>=config.boot.kernel_size;
            bool observe_wait=payload_read ? !have_payload_wait_snapshot :
                !config.boot.kernel_size && !halt_exits;
            if (observe_wait && pm_base && port==(uint32_t)pm_base+8u &&
                ...
                diagnostic_chain(&memory,guest_cr3,regs.ebp);
                have_wait_snapshot=true;
                if (payload_read) have_payload_wait_snapshot=true;
            }
