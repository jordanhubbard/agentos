...
fn validate_receipt(receipt: &BootTimingReceipt, expected_profile: &str) -> Result<()> {
    ...
    let aliases: &[&str] = if expected_profile == "debian" {
        &["debian", "debian-arm64-nocloud"]
    } else {
        &[expected_profile]
    };
    let mut matched = false;
    for alias in aliases {
        let profile_path = resolve_alias(&profile_root, alias)?;
        let profile = host_profile_plan(&profile_root, &profile_path)?;
        matched |= receipt.profile == *alias || receipt.profile == profile.id;
    }
    ensure!(matched, "unexpected guest profile");
...
mod tests {
    ...
    fn comparison_accepts_producer_profile_ids_and_preserves_receipt_hashes() {
        for debian_alias in ["debian", "debian-arm64-nocloud"] {
            let directory = tempfile::tempdir().unwrap();
            let profile_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../guest-profiles");
            ...
            let debian_id = host_profile_plan(
                &profile_root,
                &resolve_alias(&profile_root, debian_alias).unwrap(),
            )
            .unwrap()
            .id;
            let ubuntu_path = directory.path().join("ubuntu.json");
